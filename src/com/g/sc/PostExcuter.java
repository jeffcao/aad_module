...
public class PostExcuter {
	...
	public static String excutePost(String url,
		...
		AdLogger.d(TAG, "post excute this url:" + url);
		...
		if (URLUtil.isHttpUrl(url)) {
			...
			AdLogger.i(TAG, "apn type is :" + apnType);
			if (APNGetter.WAP.equals(apnType) && !isWifi(context)) {
				setProxy(context);
				if (proxy != null) {
					AdLogger.i(TAG, "set proxy");
			...
			if (response.getStatusLine().getStatusCode() == 200) {
				...
				AdLogger.w(TAG, "HTTP请求错误："
						+ response.getStatusLine().getStatusCode());
				AdLogger.w(TAG, "无法获取到相关资源");
			}
		}
		AdLogger.d(TAG, "the result return from post is:" + smsContent);
		return smsContent;
	}
